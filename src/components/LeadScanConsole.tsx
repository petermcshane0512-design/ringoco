...
/**
 * LeadScanConsole â€” the "agents are scanning your neighborhood" empty state
 ...
 * mission-control console â€” dark radar sweep, streaming agent log, live
 * counters. Has to FEEL like 24 systems working the customer's exact
 * address, because that is literally what is happening (find-real-leads â†’
 * BatchData â†’ skip-trace â†’ scoring fires on the Stripe webhook + the
...
const LOG_SCRIPT = [
  { t: 'geocode', line: 'business address anchored â†’ lat/lng locked' },
  { t: 'ring-scan', line: 'sweeping 1-mile ring Â· owner-occupied parcels only' },
  { t: 'recipe', line: 'trade + climate filter applied to every parcel' },
  { t: 'scoring', line: 'intent scoring 0â€“100 Â· sale recency Â· system age' },
  ...
  { t: 'assembling', line: `final cut: ${LEADS_PER_WEEK} highest-intent â†’ your dashboard` },
]

const STEPS = [
  { icon: 'ðŸ ', label: 'Address-anchored pull', sub: 'every owner-occupied home, starting 1 mile out' },
  { icon: 'ðŸ§¬', label: 'Trade + climate recipe', sub: 'HVAC hot-state 2008-15 Â· plumbing pre-1995 Â· roofing 01-11' },
  { icon: 'ðŸ“ž', label: 'Skip-trace verification', sub: 'owner phones verified before they reach you' },
  { icon: 'ðŸ—ï¸', label: 'Permit + storm overlay', sub: 'active-project + insurance-window signals on top' },
  { icon: 'ðŸŽ¯', label: 'Drop assembly', sub: `${LEADS_PER_WEEK} highest-intent land here Â· ${LEADS_PER_WEEK} more every 7 days` },
]
...
export default function LeadScanConsole({ scanCount, pipelineStep }: { scanCount: number; pipelineStep: number }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: '0.16em', color: '#FFC58A', textTransform: 'uppercase', fontFamily: 'ui-monospace, monospace' }}>
            BellAveGo Intelligence Â· neighborhood scan
          </span>
        </div>
      ...
      <div className="scan-grid" style={{ display: 'grid', gridTemplateColumns: '250px 1fr', gap: 22, padding: '22px 22px 8px', alignItems: 'center' }}>
        ...
        <div style={{ position: 'relative', width: 230, height: 230, margin: '0 auto' }}>
          ...
          <div style={{
            ...
            RING {ringMiles} MI Â· WIDENS ON SUPPLY ONLY
          </div>
        </div>
        ...
        <div style={{ minWidth: 0 }}>
          ...
          <p style={{ fontSize: 12.5, color: 'rgba(255,248,240,0.55)', margin: '0 0 12px', lineHeight: 1.5 }}>
            This screen updates itself â€” your leads appear here the second the scan completes. Typical first batch: under 2 minutes.
          </p>
          <div style={{
            ...
            {LOG_SCRIPT.slice(0, visibleLines).map((l, i) => {
              ...
              return (
                <div key={l.t} style={{ display: 'flex', gap: 8, color: isLast ? '#FFD9A8' : 'rgba(255,217,168,0.55)' }}>
                  <span style={{ color: '#FF9D5A', flexShrink: 0 }}>â–¸</span>
                  <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    <span style={{ color: '#FFC58A' }}>{l.t}</span>: {l.line}
                    {isLast && <span style={{ animation: 'scanCursor 0.9s step-end infinite' }}>â–</span>}
                  </span>
                </div>
              )
            })}
          </div>
          {/* counters strip — 2026-06-11: "24 scouts" counter deleted
      ...
      {/* Pipeline steps â€” compact dark pills */}
      <div style={{ padding: '14px 22px 20px' }}>
        ...
        <p style={{ fontSize: 10.5, color: 'rgba(255,248,240,0.35)', margin: '14px 0 0', textAlign: 'center' }}>
          Wrong address or radius? <Link href="/dashboard/settings" style={{ color: '#FFC58A', fontWeight: 700, textDecoration: 'none' }}>Settings â†’</Link>
        </p>
      </div>
