    ...
    <div style={{
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '60px 64px', position: 'relative', zIndex: 1 }}>
        ...
        <h1 style={{ fontSize: 'clamp(26px, 3vw, 40px)', fontWeight: 900, color: '#fff', lineHeight: 1.15, letterSpacing: '-0.03em', margin: '0 0 14px' }}>
          ...
          <span style={{ color: '#18AFA8' }}>is waiting for you.</span>
        </h1>
        <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 15, lineHeight: 1.65, maxWidth: 360, margin: '0 0 40px' }}>
          Sign in to access your dashboard, call logs, jobs, and everything your business runs on.
        </p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {[
            { icon: '📞', text: 'Every call answered, every time' },
            { icon: '📅', text: 'Jobs booked automatically to your calendar' },
            { icon: '💰', text: 'Revenue dashboard + same-day payments' },
            { icon: '🛡️', text: 'No missed call ever costs you a job again' },
          ].map(f => (
        ...
        <div style={{ marginTop: 48, paddingTop: 32, borderTop: '1px solid rgba(255,255,255,0.07)' }}>
          <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.25)', margin: 0 }}>$97/mo · No contracts · Cancel anytime</p>
        </div>
      </div>
