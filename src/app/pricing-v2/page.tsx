...
/**
 * PREVIEW PAGE — not linked from the live site.
 * View at localhost:3000/pricing-v2 to evaluate the new four-tier structure
 * ($397 / $797 / $1,997 / $2,497-per-location) before swapping production /pricing.
 *
 * All buttons route to mailto: — no Stripe price IDs wired yet on purpose.
 * When approved: rename this file to /pricing/page.tsx, create the new Stripe
 * prices, and update src/lib/pricing.ts (the dedup module from the earlier audit).
 */
...
const PLANS: Plan[] = [
  ...
  },
  {
    ...
    features: [
      ...
      { label: 'Auto-confirm mode — AI books without approval after 30-day trust window', auto: true },
  ...
  },
  {
    ...
    features: [
      ...
      { label: 'AI Ad Generator + Manager — Google + Meta ad creative weekly from your own call transcripts', auto: true },
      { label: 'AI Lead Sourcing — new-homeowner records, permits, severe-weather alerts → outbound SMS', auto: true },
      { label: 'AI Past-Customer Reactivation — drip campaigns to dormant customers', auto: true },
      { label: 'AI Google Business Profile Manager — posts, photos, local-pack rank tracking', auto: true },
      { label: 'AI Competitor Watcher — daily intel on 5 competitors in your service area', auto: true },
      { label: 'AI Local SEO — weekly blog posts ranking "best [trade] [city]"', auto: true },
      ...
      { label: 'API access for custom integrations', auto: true },
    ],
  },
]
  ...
  return (
    <main style={{ fontFamily: "'Inter', system-ui, sans-serif", background: '#F2F9F5', color: '#0B1F3A', minHeight: '100vh' }}>
      ...
      <nav style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 48px', height: 72, background: '#fff', borderBottom: '1px solid #DCE9E2', position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        <div style={{ background: '#FEF3C7', color: '#92400E', padding: '6px 14px', borderRadius: 999, fontSize: 12, fontWeight: 800, letterSpacing: '0.05em' }}>
          PREVIEW — not live pricing
        </div>
      </nav>

      <section style={{ padding: '72px 24px 32px', textAlign: 'center', maxWidth: 1200, margin: '0 auto' }}>
        <p style={{ fontSize: 13, fontWeight: 700, color: '#0AA89F', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 12 }}>Pricing v2 · proposed</p>
      ...
      <footer style={{ padding: '36px 40px', background: '#0B1F3A', textAlign: 'center' }}>
        <p style={{ margin: 0, fontSize: 12, color: '#7AAAB2' }}>PREVIEW · /pricing-v2 · not linked from live site</p>
      </footer>
    </main>
  )
}
