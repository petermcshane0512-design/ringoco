...
export default function PricingPage() {
  ...
  return (
    <main style={{ fontFamily: "'Inter', system-ui, sans-serif", background: '#F2F9F5', color: '#0B1F3A', minHeight: '100vh' }}>
      ...
      {/* "Which tier is right for me?" decision matrix — kills decision paralysis
          by pre-qualifying the buyer into their ICP before they scan feature lists. */}
      <section style={{ padding: '0 24px 36px' }}>
        <div style={{ maxWidth: 1080, margin: '0 auto' }}>
          <p style={{ textAlign: 'center', fontSize: 12, fontWeight: 800, color: '#C84B26', letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: 18 }}>Which tier is right for you?</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 14 }}>
            {[
              {
                heading: '1–2 employees',
                detail: 'You answer the phone yourself or with one assistant. Mostly service calls, cash or simple invoicing. You don\'t send formal quotes.',
                tier: 'receptionist' as Tier,
                pickLabel: 'Mission Control',
                price: 397,
                accent: '#0AA89F',
                accentSoft: 'rgba(10,168,159,0.10)',
              },
              {
                heading: '3–10 trucks',
                detail: 'Multiple techs in the field. You send 10+ quotes a month, bill on net terms, care about Google reviews. Quote Hunter + Collections pay for themselves on the first recovered job.',
                tier: 'officemgr' as Tier,
                pickLabel: 'Operator',
                price: 797,
                accent: '#E8742B',
                accentSoft: 'rgba(232,116,43,0.12)',
                popular: true,
              },
              {
                heading: '10+ trucks / Multi-Location',
                detail: 'Multiple shops, multiple service areas, multiple phone lines. You need shared dashboards, marketing operations, and a real account manager.',
                tier: 'concierge' as Tier,
                pickLabel: 'Operator + Multi-Location',
                price: null,
                accent: '#7C3AED',
              ...
              <button
                key={card.tier}
                onClick={() => document.getElementById(`plan-${card.tier}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' })}
                style={{
                  textAlign: 'left',
                  background: '#fff',
                  border: `2px solid ${card.popular ? card.accent : 'rgba(10,168,159,0.18)'}`,
                  borderRadius: 16,
                  ...
                  position: 'relative',
                  boxShadow: card.popular ? `0 12px 30px ${card.accentSoft}` : '0 2px 12px rgba(7,27,58,0.05)',
                }}
                onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.boxShadow = `0 16px 36px ${card.accentSoft}` }}
                onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = card.popular ? `0 12px 30px ${card.accentSoft}` : '0 2px 12px rgba(7,27,58,0.05)' }}
              >
                {card.popular && (
                  <div style={{ position: 'absolute', top: -10, right: 14, background: card.accent, color: '#fff', fontSize: 10, fontWeight: 800, padding: '3px 10px', borderRadius: 999, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
                    Most pick this
                  </div>
                )}
                <div style={{ fontSize: 11, fontWeight: 800, color: card.accent, letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: 6 }}>
                  If you have…
                </div>
                <div style={{ fontSize: 22, fontWeight: 900, color: '#0B1F3A', letterSpacing: '-0.4px', marginBottom: 10 }}>
                  {card.heading}
                </div>
                <div style={{ fontSize: 13, color: '#4A7A80', lineHeight: 1.55, marginBottom: 16 }}>
                  {card.detail}
                </div>
                <div style={{ paddingTop: 14, borderTop: `1px solid ${card.accentSoft}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 700, color: '#7AAAB2', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 2 }}>Best fit</div>
                    <div style={{ fontSize: 15, fontWeight: 800, color: '#0B1F3A' }}>{card.pickLabel}</div>
                  </div>
                  <div style={{ fontSize: 13, fontWeight: 800, color: card.accent }}>
                    {card.price ? `from $${card.price}/mo` : 'Custom'} →
                  </div>
                </div>
              </button>
            ))}
